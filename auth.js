/**
 * Authentication Module for Amazon PPC Campaign Builder V2.0
...
class AuthManager {
    ...
    loadUserSession() {
        try {
            ...
            let userData = localStorage.getItem('ppcBuilderV2_user');
            let isRemembered = localStorage.getItem('ppcBuilderV2_rememberMe') === 'true';
            
            if (!userData) {
                // Check sessionStorage (session login)
                userData = sessionStorage.getItem('ppcBuilderV2_user');
                isRemembered = false;
            }
    ...
    validateSession(user, isRemembered) {
        ...
        if (isRemembered) {
            ...
            return sessionStorage.getItem('ppcBuilderV2_user') !== null;
        }
    }
    ...
    requireAuth() {
        if (!this.isAuthenticated()) {
            ...
            if (currentPath !== '/signin.html') {
                sessionStorage.setItem('ppcBuilderV2_redirectAfterLogin', currentPath);
            }
    ...
    clearSession() {
        this.currentUser = null;
        localStorage.removeItem('ppcBuilderV2_user');
        localStorage.removeItem('ppcBuilderV2_rememberMe');
        sessionStorage.removeItem('ppcBuilderV2_user');
        sessionStorage.removeItem('ppcBuilderV2_redirectAfterLogin');
    }
    ...
    setupEventListeners() {
        ...
        window.addEventListener('storage', (e) => {
            if (e.key === 'ppcBuilderV2_user' || e.key === 'ppcBuilderV2_rememberMe') {
                if (e.newValue === null) {
